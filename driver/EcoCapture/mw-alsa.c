...
typedef struct _mw_card_driver {
    struct capture_channel *    capch;
...
static int _deliver_samples(mw_card_driver *drv, void *aud_data, u32 aud_len)
{
...
static int _create_resampler(mw_card_driver *drv,
        u32 sample_rate_in,
        u32 sample_rate_out)
{
...
static void _destroy_resampler(mw_card_driver *drv)
{
    if (NULL != drv->resampler) {
...
static void _audio_sample_rate_changed(mw_card_driver *drv, u32 sample_rate)
{
    mw_debug(1, "Audio Sample Rate Changed: %d\n", sample_rate);
    ...
    if (sample_rate != drv->sample_rate_out && sample_rate != 0) {
        ...
        drv->max_output_samples =
                GetMaxOutput(MWCAP_AUDIO_SAMPLES_PER_FRAME * drv->channels,
                             drv->resampler);
    }
}
...
static void _process_audio_frame(
        mw_card_driver *drv, BOOLEAN bLPCM,
        WORD wChannelValid,
        const MWCAP_AUDIO_CAPTURE_FRAME * pAudioFrame)
{
    const DWORD *pdwSamples = pAudioFrame->adwSamples;
    ...
    for (i = 0; i < MWCAP_AUDIO_SAMPLES_PER_FRAME; i++) {
        ...
        case 8:
            if (wChannelValid & 0x08) {
                asSamples[6] = (short)(pdwSamples[i * 8 + 3] >> 16);
                asSamples[7] = (short)(pdwSamples[i * 8 + 7] >> 16);
            }
            else
        ...
        case 6:
            if (wChannelValid & 0x04) {
                asSamples[4] = (short)(pdwSamples[i * 8 + 2] >> 16);
                asSamples[5] = (short)(pdwSamples[i * 8 + 6] >> 16);
            }
            else
        ...
        case 4:
            if (wChannelValid & 0x02) {
                asSamples[2] = (short)(pdwSamples[i * 8 + 1] >> 16);
                asSamples[3] = (short)(pdwSamples[i * 8 + 5] >> 16);
            }
            else
        ...
        default:
            if (wChannelValid & 0x01) {
                asSamples[0] = (short)(pdwSamples[i * 8 + 0] >> 16);
                asSamples[1] = (short)(pdwSamples[i * 8 + 4] >> 16);
            }
            else
    ...
    if (bLPCM && drv->resampler) {
        if (drv->max_output_samples + drv->num_output_samples <= drv->resampled_buf_size / sizeof(short))
            drv->num_output_samples += Resample(drv->audio_samples,
                    MWCAP_AUDIO_SAMPLES_PER_FRAME * nChannels,
                    &_resampled_buf[drv->num_output_samples],
                    drv->resampler);
    } else if (MWCAP_AUDIO_SAMPLES_PER_FRAME * nChannels + drv->num_output_samples <=
            drv->resampled_buf_size / sizeof(short)) {
        memcpy(&_resampled_buf[drv->num_output_samples], drv->audio_samples,
                MWCAP_AUDIO_SAMPLES_PER_FRAME * sizeof(short) * nChannels);
        drv->num_output_samples += MWCAP_AUDIO_SAMPLES_PER_FRAME * nChannels;
    }
...
static int audio_capture_thread_proc(void *data)
{
    ...
    capture_channel_AcquireAudioCapture(drv->capch);

    capture_channel_GetAudioSignalStatus(drv->capch, &audio_signal_status);
    _audio_sample_rate_changed(drv, audio_signal_status.dwSampleRate);

    pre_block_id = capture_channel_GetCompletedAudioBlockId(drv->capch);
    ...
    while (1) {
        ...
        status_bits = notify_event_mgr_get_status_bits(drv->m_pNotifyEventManager, drv->notify_event);

        if (status_bits & MWCAP_NOTIFY_AUDIO_SIGNAL_CHANGE) {
            capture_channel_GetAudioSignalStatus(drv->capch, &audio_signal_status);

            mw_debug(20, "LPCM: %d, Sample Rate: %d, Bits Per Sample: %d, Channel Flags: %d\n",
                    audio_signal_status.bLPCM,
                    audio_signal_status.dwSampleRate,
                    audio_signal_status.cBitsPerSample,
        ...
        if (status_bits & MWCAP_NOTIFY_AUDIO_INPUT_RESET) {
            pre_block_id = -1;
            mw_debug(5, "Reset Audio Buffer\n");
        }

        if (status_bits & MWCAP_NOTIFY_AUDIO_FRAME_BUFFERED) {
            int new_block_id = capture_channel_GetCompletedAudioBlockId(drv->capch);

            if (new_block_id >= 0) {
                while (pre_block_id != new_block_id) {

                    pre_block_id = (pre_block_id + 1) % AUDIO_FRAME_COUNT;
                    capture_channel_GetNormalizedAudioCaptureFrame(
                                drv->capch, pre_block_id, audio_frame);

                    _process_audio_frame(drv, audio_signal_status.bLPCM,
                                         audio_signal_status.wChannelValid,
                                         audio_frame);
                }
            }
    ...
    capture_channel_ReleaseAudioCapture(drv->capch);
    ...
    mw_debug(0, "Stop audio capture\n");

    return 0;
}

static int _start_audio_capture(mw_card_driver *drv,
        u32 samples_per_sec, unsigned int channels)
{
    ...
    drv->notify_event = notify_event_mgr_add(
                drv->m_pNotifyEventManager,
    ...
    drv->kthread = kthread_run(audio_capture_thread_proc, drv, "MW-ALSA");
    if (IS_ERR_OR_NULL(drv->kthread)) {
        mw_debug(0, "Create kernel thread MW-ALSA Error: %m\n");
        goto del_notify;
    }
...
del_notify:
    notify_event_mgr_remove(drv->m_pNotifyEventManager, drv->notify_event);
free_buf:
...
static void _stop_audio_capture(mw_card_driver *drv)
{
...
{
    mw_card_driver *drv = container_of(work, mw_card_driver, start_work);
    struct snd_pcm_substream *substream = drv->substream;

    mw_debug(1, "START work\n");
...
{
    mw_card_driver *drv = container_of(work, mw_card_driver, stop_work);

    mw_debug(1, "STOP work\n");
...
{
    mw_card_driver *drv = snd_pcm_substream_chip(substream);
...
static int alsa_create_pcm(struct snd_card *card, mw_card_driver *drv)
{
    int err;
    struct snd_pcm *pcm;
...
{
    mw_card_driver *drv = snd_kcontrol_chip(kcontrol);
    int i;

    for (i = 0; i < 8; i++) {
        value->value.integer.value[i] = drv->m_anVolume[i];
    }
    ...
    struct snd_ctl_elem_value *value)
{
    ...
    for (i = 0; i < 8; i++) {
        drv->m_anVolume[i] = _limit(value->value.integer.value[i], -90 * 0x10000, 10 * 0x10000);
    }
    ...
    struct snd_ctl_elem_value *value)
{
    ...
    for (i = 0; i < 8; i++) {
// TODO parameter
        value->value.integer.value[i] = !drv->m_abMute[i];
    }
    ...
    struct snd_ctl_elem_value *value)
{
    ...
    for (i = 0; i < 8; i++) {
        drv->m_abMute[i] = !value->value.integer.value[i];
    }
...
int alsa_card_init(struct capture_channel *capch, struct snd_card **pcard, void *parent_dev)
{
    int err;
    mw_card_driver *drv;
    ...
    err = snd_card_new(parent_dev, index[audio_devno], id[audio_devno], THIS_MODULE, sizeof(mw_card_driver), &card);
#elif defined(RHEL_RELEASE_CODE)
#if (RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(7, 1))
    err = snd_card_new(parent_dev, index[audio_devno], id[audio_devno], THIS_MODULE, sizeof(mw_card_driver), &card);
#else
    err = snd_card_create(index[audio_devno], id[audio_devno], THIS_MODULE, sizeof(mw_card_driver), &card);
    if (err == 0)
        snd_card_set_dev(card, parent_dev);
#endif
#else
    err = snd_card_create(index[audio_devno], id[audio_devno], THIS_MODULE, sizeof(mw_card_driver), &card);
    if (err == 0)
    ...
    drv->capch = capch;
    drv->m_pNotifyEventManager = capture_channel_GetNotifyEventManager(capch);

    // Audio controls
    for (i = 0; i < 8; i++) {
        drv->m_abMute[i] = FALSE;
        drv->m_anVolume[i] = 0;
    }
    ...
    if (capture_channel_GetChannelName(drv->capch, card->shortname, sizeof(card->shortname)) <= 0)
...
{
    mw_card_driver *drv = card->private_data;
