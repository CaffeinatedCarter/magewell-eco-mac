...
typedef enum _TIMESTAMP_MODE {
    TIMESTAMP_MODE_SYS_NORMAL,
    TIMESTAMP_MODE_SYS_START,
    TIMESTAMP_MODE_CARD_START,
    TIMESTAMP_MODE_MAX = TIMESTAMP_MODE_CARD_START
} TIMESTAMP_MODE;
...
int parameter_manager_init(struct parameter_t *par, PARAMETER_EDID_TYPE nEDIDType);
...
// Audio controls
static inline BOOLEAN parameter_GetMute(struct parameter_t *par, LONG lChannel)
{
    if (lChannel >= ARRAY_SIZE(par->m_abMute) || lChannel < 0)
        return FALSE;

    return par->m_abMute[lChannel];
}

static inline int parameter_GetVolume(struct parameter_t *par, int nChannel)
{
    if (nChannel >= ARRAY_SIZE(par->m_anVolume) || nChannel < 0)
        return 0;

    return par->m_anVolume[nChannel];
}

static inline void parameter_SetMute(struct parameter_t *par, LONG lChannel, BOOLEAN bMute)
{
    if (lChannel >= ARRAY_SIZE(par->m_abMute) || lChannel < 0)
        return;

    par->m_abMute[lChannel] = !!bMute;
}

static inline void parameter_SetVolume(struct parameter_t *par, int nChannel, int nVolume)
{
    if (nChannel >= ARRAY_SIZE(par->m_anVolume) || nChannel < 0)
        return;

    par->m_anVolume[nChannel] = _limit(nVolume, -90 * 0x10000, 10 * 0x10000);
}
...
static inline void parameter_SetTimestampMode(struct parameter_t *par,
                                                        TIMESTAMP_MODE mode)
{
    par->m_nTimestampMode = _limit(mode, TIMESTAMP_MODE_SYS_NORMAL, TIMESTAMP_MODE_MAX);
}
