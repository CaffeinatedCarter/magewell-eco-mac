...
int os_atomic_inc(os_atomic_t *v)
{
    return atomic_inc_return(v);
}

int os_atomic_dec(os_atomic_t *v)
{
    return atomic_dec_return(v);
}

int os_atomic_add(os_atomic_t *v, int i)
{
    return atomic_add_return(i, v);
}

int os_atomic_sub(os_atomic_t *v, int i)
{
...
os_event_t os_event_alloc(void)
{
    ...
    os_atomic_set(&event->refcount, 1);
...
{
    if (os_atomic_dec(&event->refcount) == 0) {
                                     ...
                                     const char *name)
{
    ...
    if (IS_ERR_OR_NULL(thread->kthread)) {
        os_event_release(thread->wait_exit);
        os_free(thread);
        return NULL;
    }
...
/* mem io */
uint32_t pci_read_reg32(volatile void *addr, os_pci_dev_t pci_dev) {
    return readl(addr);
}

void pci_write_reg32(volatile void *addr, uint32_t val, os_pci_dev_t pci_dev) {
    writel(val, addr);
}
...
void *os_malloc(unsigned int size)
{
    ...
    // 4k plus dma need align to 16 bytes
    alignment = sizeof(*mhead) <= 16 ? 16 : 32;
    memsize = alignment + size;
    ...
    if (memsize <= kmalloc_max_size) {
        addr_start = kmalloc(memsize, GFP_KERNEL);
        mtype = OS_LINUX_MEM_TYPE_KMALLOC;
    }

    if (addr_start == NULL) {
        memsize = PAGE_ALIGN(memsize);
        addr_start = vmalloc(memsize);
    ...
    addr_data = addr_start + alignment;
    mhead = (struct _mhead *)(addr_data);
    mhead--;

    mhead->maddr = addr_start;
    mhead->msize = memsize;
    mhead->mtype = mtype;
...
static inline bool is_pow_of_two(unsigned long num)
{
    if (num < 1) return false;
    return (num & (num - 1)) == 0;
}

void *os_malloc_aligned(unsigned int size, unsigned int alignment)
{
    void*           addr_start = NULL;
    void*           addr_data = NULL;
    struct _mhead *  mhead = NULL;
    ...
    if (size == 0 || !is_pow_of_two(alignment) || alignment < 32) {
        return (0);
    }

    memsize = alignment + size;
    memsize = PAGE_ALIGN(memsize);
    addr_start = (struct _mhead *)vmalloc(memsize);
    ...
    mhead->maddr = addr_start;
    mhead->msize = memsize;
    mhead->mtype = OS_LINUX_MEM_TYPE_VMALLOC;
...
size_t os_mem_get_actual_size(void *address)
{
    ...
    if (hdr->mtype == OS_LINUX_MEM_TYPE_KMALLOC)
        return ksize(hdr->maddr) - (address - hdr->maddr);
    else
        return hdr->msize - (address - hdr->maddr);
}
